...
const Benefits = () => {
  return (
    <div className="container mx-auto my-6 py-12 p-6">
      <h2 className="text-2xl md:text-4xl text-left text-[#564E4E] sm:block hidden font-medium mb-6">Why Choosing Us</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {benefits.map(benefit => (
          <div key={benefit.id} className="text-left bg-white rounded-lg  hover:drop-shadow-2xl overflow-hidden hover:border transition duration-300 hover:border-glow hover:border-[#FADCD9]">
            <img src={benefit.image} alt={benefit.name} className="sm:w-16 sm:h-16 w-8 h-8 object-cover ml-4 mt-4" />
            <div className="p-4">
              <h3 className="sm:text-lg text-xs font-medium mb-2">{benefit.name}</h3>
              <p className="text-gray-600 sm:text-lg text-xs mb-2">{benefit.description}</p>
              <button className='text-[#F79489] sm:text-lg text-xs'>more info</button>
            </div>
          </div>
        ))}
      </div>
