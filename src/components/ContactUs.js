...
const ContactUs = () => {
  return (
    <div className="container mt-12 mx-auto flex flex-col p-6 md:flex-row items-center justify-between py-10">
      ...
      {/* Contact Form */}
      <div className="flex flex-col mb-12 text-[#564E4E] md:w-1/2 gap-y-10">
        <h1 className="text-left text-2xl md:text-4xl">Make your room feel homey</h1>
        <p className='text-left  text-md md:text-lg'> We help people not only to make the room at home better, but we help people to feel a more homey and comfortable room, you don’t have to worry about the result because of these interiors are made by people who are professionals in their dields with an elegant and lucuriois style and with premium quality materials 
           <br/><br/> Contact us if you are interested in working with us </p>
        <div className="flex items-center lg:w-1/2 border border-[#FADCD9] text-[#FEFCFB]  rounded-full overflow-hidden relative">
          <input
            type="email" placeholder="Insert your email address" style={{ backgroundColor: '#FEFCFB' }}
            className="rounded-full py-2 px-4 w-full outline-[#FADCD9] text-[#F79489] placeholder:text-[#F79489] placeholder:text-xs" />
          <button className="p-4 scale-[0.88] absolute right-1 top-0 h-full px-4 bg-[#FADCD9] rounded-full flex items-center">
